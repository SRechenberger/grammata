...
where
    ...
    popBacktrackPoint :: (Monad monad) 
        ...
        -> monad (Maybe (LStorage state monad, (state, monad ())))  -- ^ Pair of the updated storage and the return point.
    popBacktrackPoint (LStorage []) = return Nothing
    popBacktrackPoint (LStorage (rpt:rpts)) = return $ Just (LStorage rpts, rpt)